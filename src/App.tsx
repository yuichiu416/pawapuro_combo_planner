...
import { 
  ChevronLeft, ChevronRight, Save, Loader2, Clock, SearchX, 
  Users, List, BarChart3, X 
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const { 
    ...
    searchTerm, setSearchTerm, filteredComboIds = [], filterRelatedOnly,
    toggleRelatedFilter, filterNoKanji, toggleKanjiFilter, handleSave, 
    isSyncing, lastSaved, libraryGroups = { withCombo: [], noCombo: [] },
    ...
    >
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside className={cn(
          ...
          <div className={cn("h-full w-[24rem] overflow-hidden", isSidebarCollapsed && "opacity-0 pointer-events-none")}>
            <CharacterSidebar 
              searchTerm={searchTerm} setSearchTerm={setSearchTerm} 
              posFilter={posFilter} setPosFilter={setPosFilter} 
              mapFilter={mapFilter} setMapFilter={setMapFilter} 
              filterNoKanji={filterNoKanji} toggleKanjiFilter={toggleKanjiFilter} 
        ...
        <div className={cn(
          ...
          <div className={cn(
            ...
            <div className="p-4 border-b flex justify-between items-center">
              <span className="font-black italic uppercase tracking-tighter">Character Library</span>
              <button data-testid="mobile-character-sidebar-close-btn" onClick={() => setActiveMobileTab('planner')} className="p-2 bg-slate-100 rounded-full"><X size={20}/></button>
            </div>
            <CharacterSidebar 
              searchTerm={searchTerm} setSearchTerm={setSearchTerm} 
              posFilter={posFilter} setPosFilter={setPosFilter} 
        ...
        <main className={cn(
          ...
          <div className="max-w-7xl mx-auto space-y-8 md:space-y-12">
            <Header 
              showPositionIcon={showPositionIcon} setShowPositionIcon={setShowPositionIcon} 
              filterRelatedOnly={filterRelatedOnly} toggleRelatedFilter={handleToggleRelated} 
              toggleAllByType={toggleAllByType} clearAll={clearAll} 
            ...
            <div className="space-y-12 md:space-y-16">
              {Object.entries(mapsData).map(([mapName, data]) => {
                ...
                return (
                  <MapSection 
                    key={mapName} mapName={mapName} combos={mapCombos} 
                    searchTerm={searchTerm} selectedComboIds={selectedComboIds} 
                    toggleCombo={toggleCombo} ownedChars={ownedChars} 
                    toggleCharacter={toggleCharacter} onAddCharacters={handleAddCharacters} 
                    getImagePath={getImagePath} showPositionIcon={showPositionIcon} 
              ...
              {filterRelatedOnly && filteredComboIds.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 bg-white/50 rounded-3xl border-2 border-dashed border-slate-200 px-6 text-center">
                  ...
                  <h3 className="text-xl font-black italic uppercase text-slate-400">No related combos</h3>
                  <p className="text-sm font-bold text-slate-400/80 uppercase mt-1">Try selecting more characters</p>
                </div>
              )}
            </div>
          </div>
...
const MobileNavBtn = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
  ...
  >
    <div className={cn("p-2 rounded-xl transition-colors", active ? "bg-blue-50" : "bg-transparent")}>
      {icon}
    </div>
